...
UIvertexes VulkanSamplerDescriptor::getUIvertexes(std::string const& text, vec2i const& origin, uint32_t index, bool isRightAligned) const noexcept
{
	assert(index < this->textures.size() && "Texture index not found in descriptor");

	return this->textures[index]->getUIvertexes(text, origin, isRightAligned);	
}
